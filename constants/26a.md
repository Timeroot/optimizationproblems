...
The constant $C_{26}$ is the **largest** $c$ such that
...
The (symmetric) **Mahler conjecture** predicts that $C_{26}=4$, with extremisers given by **Hanner polytopes** (in particular, the cube and cross-polytope).

## Known upper bounds

| Bound | Reference | Comments |
|---|---|---|
| $4$ | Trivial | For the cube $B_\infty^n=[-1,1]^n$ one has $\mathrm{Vol}(B_\infty^n)=2^n$ and $\mathrm{Vol}\big((B_\infty^n)^\circ\big)=2^n/n!$, hence $M(B_\infty^n)=4^n$ and $C_{26}\le 4$. Conjecturally, this is sharp (Mahler conjecture). |
